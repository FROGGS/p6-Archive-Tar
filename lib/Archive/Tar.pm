...
our ($DEBUG, $error, $VERSION, $WARN, $FOLLOW_SYMLINK, $CHOWN, $CHMOD,
...
method _read_tar($handle, *%opts) {
    ...
    LOOP: while $chunk = $handle.read( HEAD ) {
        ...
        #~ if ( ref($handle) ne 'IO::Zlib' ) {
            #~ local $@;
            $offset = try { $handle.tell } || 'unknown';
            #~ $@ = '';
        #~ }
        #~ else {
            #~ $offset = 'unknown';
...
method create_archive($file, $gzip = 0, *@files) {
    return unless defined $file;
...
method iter($filename, $compressed = 0, *%opts) {
    ...
    return sub {
        return @data.shift if @data;       # more than one file returned?
        return             unless $handle; # handle exhausted?

        ### read data, should only return file
        my $tarfile = self._read_tar($handle, |%opts, :limit(1) );
...
method list_archive($file, $gzip = 0) {
    return unless defined $file;
...
sub no_string_support {
    fail "You have to install IO::String to support writing archives to strings";
}

sub _symlinks_resolver($src, $trg) {
    my @src = split /<[\/\\]>/, $src;
    my @trg = split /<[\/\\]>/, $trg;
