...
#~ method new(:$chunk, :$file, :$data, *@args) {
    #~ my $new = self.CREATE;
    #~ $new.BUILD(:$chunk, :$file, :$data, |@args);
#~ }

#~ submethod BUILD(:$chunk, :$file, :$data, *@args) {
    #~ my $obj = $chunk ?? self.WHAT._new_from_chunk( $chunk, |@args ) !!
              #~ $file  ?? self.WHAT._new_from_file(  $file,  |@args ) !!
              #~ $data  ?? self.WHAT._new_from_data(  $data,  |@args ) !!
              #~ Nil;
...
method new_from_chunk($chunk, *@args) {
    return unless $chunk;    # 512 bytes of tar header
    my %hash = @args;
    ...
    my %args = %hash.grep( *.value.defined );
    ...
    if substr(%entry<size>, 0, 1) eq "\x80" {	# binary size extension for files >8gigs (> octal 77777777777777)	# cdrake
      ...
      # that seems unlikely to ever be needed - the numbers are just too big...) # cdrake
      %entry<size> = @sz[4] + (2**32) * @sz[3] + @sz[2] * (2**64);
    }
    else {	# cdrake
      %entry<size> ~~ s/\D+$//; %entry<size> = %entry<size> ?? :8(~%entry<size>) !! 0;	# cdrake
    }	# cdrake

    my $obj = self.new(|%entry, |%args);
